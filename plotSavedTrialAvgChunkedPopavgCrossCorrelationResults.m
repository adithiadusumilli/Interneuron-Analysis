function plotSavedTrialAvgChunkedPopavgCrossCorrelationResults(saveFile)
% plots saved trial-averaged chunked population-averaged cross-correlation outputs
% run: plotSavedTrialAvgChunkedPopavgCrossCorrelationResults("C:\Users\mirilab\Documents\GlobusTransfer\concatCrossCorr_trialavg_chunked_popavg_ALL_SESSIONS.mat")
...
title('Trial-Averaged Cortex Peak Lags with 95% Permutation Lag Bounds', 'FontSize', titleFont);
box off;
grid on;
set(gca, 'FontSize', tickFont, 'LineWidth', 1, 'TickDir', 'out');
...
figure('Name','Trial-Averaged Chunked XC Peak Lag Permutation Distributions', 'Color','w');
tile_lay2 = tiledlayout(1, nSess, 'TileSpacing','compact','Padding','compact');
title(tile_lay2, 'Trial-Averaged Actual Peak Lags vs. Permutation Distribution', 'FontSize', titleFont);
...
for s = 1:nSess
    ...
    if ~legendSet
        ...
        sharedLegendHandles(2) = patch(nan, nan, permHistColor, 'EdgeColor', 'none');
...
figure('Name','Trial-Averaged Chunked XC Peak Lag Permutations Combined', 'Color','w');
hold on;
