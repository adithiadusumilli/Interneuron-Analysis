function plotSavedIntPyrLagImbalanceMAFDR_NoChunk(resultsFile, combinedMatFile)
% plots saved no-chunk pairwise significance results from
...
summaryActualLagImbalance = nan(nSess,1);
...
for s = 1:nSess
    ...
    if ~isempty(sigPairsIdx)
        ...
        if ~isempty(lagsVec) && ~isempty(xcVec)
            ...
            hCurve = plot(lagsVec, xcVec, '-', 'Color', [0.2 0.2 0.2], 'LineWidth', 1.5);
    ...
    figure('Name', sprintf('%s Lag Imbalance Relative To Null Distribution', animalID), 'Color', 'w');

    if ~isempty(validNullLagImbalance)
        histogram(validNullLagImbalance, 30, 'EdgeColor', 'none', 'FaceColor', nullColor);
        hold on;
        hActual = xline(actual.lagImbalance, 'r', 'LineWidth', 2);
        ...
        legend([hNullDummy hActual hCI], ...
            {'Null Lag-Imbalance Distribution', 'Actual Lag Imbalance', '95% Null Interval'}, ...
...
for s = 1:nSess
    ...
    hDot = plot(xActual, summaryActualLagImbalance(s), ...
    ...
    ylabel('Lag Imbalance', 'FontSize', 16);
    ...
    if s == 1 && isgraphics(hCI)
        lgd = legend([hDot hCI], ...
            {'Actual Lag Imbalance', '95% Null Interval'}, ...
