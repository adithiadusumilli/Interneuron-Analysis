function runIntPyrLagSkewBHFDRPermutationTest(saveFile, analysisType, alpha, nNullDraws, flag)
% runs int-vs-pyr-only BH/FDR analysis and skew permutation testing
% for either no-chunk or chunked pairwise saved results

% updated logic:
%   - compute empirical p-values for all true int x pyr pairs
%   - apply BH/FDR to all valid p-values
%   - call pairs significant if p <= BH cutoff
%   - compute skew = (mean - median) / std from BH-significant lags
...
%   - compute p-values, BH/FDR significance, significant lags, and skew
...
results.flag = flag;
results.fdrMethod = 'BH_all_valid_pvalues';
...
for s = 1:nSess
    ...
    actual = computePairSetStatsBHFDR(peakCorr, peakLag, nullCorr, actualRows, actualCols, alpha, flag);

    fprintf('%s actual: nominal pairs = %d | valid tests = %d | uncorr p<=%.3f = %d | BH sig = %d | BH cutoff = %.6f | skew = %.6f\n', ...
        animalID, numel(actualRows), actual.nValidTests, alpha, actual.nSigUncorr, actual.nSigFDR, actual.fdrCutoff, actual.skew);
    ...
    for r = 1:nNullDraws
        ...
        nullDraw = computePairSetStatsBHFDR(peakCorr, peakLag, nullCorr, drawRows, drawCols, alpha, flag);
    ...
    figure('Name', sprintf('%s actual int-pyr significant lags (BH)', animalID), 'Color', 'w');
    ...
    xlabel('Peak Lag (s)');
    ylabel('Count');
    title(sprintf('%s actual int-pyr BH-significant lags | uncorr=%d | BH sig=%d | skew=%.4f', ...
    ...
    figure('Name', sprintf('%s null significant lags (BH)', animalID), 'Color', 'w');
    ...
    figure('Name', sprintf('%s skew null distribution (BH)', animalID), 'Color', 'w');
...
outFile = fullfile(folderPath, sprintf('%s_intPyrSkewBHFDR_%s.mat', baseName, char(analysisType)));
...
% ============================================================
function out = computePairSetStatsBHFDR(realMat, lagMat, nullMat, rows, cols, alpha, flag)
% computes pairwise empirical p-values, BH/FDR significance on all valid p-values,
% and skew for an arbitrary list of selected pairs
