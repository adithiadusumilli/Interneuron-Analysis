function plotSavedIntPyrLagSkew5Std_NoChunk(resultsFile, combinedMatFile)
% plots saved no-chunk pairwise significance results from runIntPyrLagSkew5StdPermutationTest
...
for s = 1:nSess
    ...
    if isempty(actual) || ~isfield(actual, 'sig5StdMask')
    ...
    for k = 1:numel(actual.rows)
        ...
        if c >= 1 && c <= nPyr && actual.sig5StdMask(k)
    ...
    title(sprintf('%s Significant Pairwise Peak Lag Map (%d / %d Pairs)', ...
        animalID, actual.nSig5Std, actual.nPairsNominal), ...
    ...
    sigPairsIdx = find(actual.sig5StdMask(:));
    ...
    if ~isempty(actual.sigLagVec) && ~isempty(actual.realVals)
        sigCorrVec = actual.realVals(actual.sig5StdMask);
