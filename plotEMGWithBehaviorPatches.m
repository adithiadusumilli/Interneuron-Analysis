...
switch lower(labelType)
    ...
    case "classifier"
        ...
        nColors = 11;
    otherwise
        error('labeltype "%s" not supported. use "classifier", "manual", or "umap".', char(labelType));
end
...
% plot emg first (save handle so we can force it on top later)
...
if nColors == 11
    cmap = [
        ...
        0.80 0.35 0.00   % walkgrid (dark orange)
    ];
else
    cmap = [
        0.00 0.45 0.85
