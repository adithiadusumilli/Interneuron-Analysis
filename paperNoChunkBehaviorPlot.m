function paperNoChunkBehaviorPlot(noChunkFile)
...
% Sentence-case behavior labels for plots
behNamesPretty = { ...
    'Climb down', ...
    'Climb up', ...
    'Eating', ...
...
R.sessions = R.sessions(keepSess);
...
for s = 1:nSess
    ...
    for k = 1:numel(behNums)
        ...
        if isempty(thisBeh) || ...
                ~isfield(thisBeh, 'lagsSec') || ...
                isempty(thisBeh.lagsSec)
...
%% =========================================================
%  precompute common x-limits and bin edges for permutation
%  histograms per animal
% ==========================================================
...
for s = 1:nSess
    ...
    for k = 1:numel(behNums)
        ...
        if isfield(thisBeh, 'permPeakLags') && ...
        ...
        if isfield(thisBeh, 'peakLagSec') && ...
                ~isempty(thisBeh.peakLagSec) && ...
                ~isnan(thisBeh.peakLagSec)

            allActualVals = [ ...
                allActualVals; ...
                thisBeh.peakLagSec]; %#ok<AGROW>
        end
    end
...
for s = 1:nSess
    ...
    title(tile_lay1, ...
        sprintf('%s M1 lag vs. correlation', animalIDs{s}), ...
        'FontSize', titleFont, ...
        'FontWeight', 'bold');
    ...
    for k = 1:numel(behNums)
        ...
        nexttile(tile_lay1, k);
        hold on;

        if isempty(thisBeh) || ...
                ~isfield(thisBeh, 'timeIdx') || ...
                isempty(thisBeh.timeIdx) || ...
                ~isfield(thisBeh, 'xc') || ...
                isempty(thisBeh.xc) || ...
                ~isfield(thisBeh, 'lagsSec') || ...
                isempty(thisBeh.lagsSec)

            title(behNamesPretty{k}, ...
        ...
        % Shift control correlation bounds
        hCorr25 = gobjects(1);
        hCorr97 = gobjects(1);
...
for s = 1:nSess
    ...
    for k = 1:numel(behNums)
        ...
        if ~legendSet
            ...
            % Dummy patch so blue histogram appears correctly in legend
            hHistLegend = patch( ...
                nan, ...
                nan, ...
                permHistColor, ...
