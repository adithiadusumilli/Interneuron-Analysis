function runNonChunkedXcorrPerBehavior_saveOnly_getMouseDataNames_fullperm(mouseIDs, baseSessionNames, probeRegions, labelType, behaviors, maxLagSecs, numShifts, numPerms)
...
%   numPerms  = 300
%
% The 300 neuron-label permutations are preserved in their ORIGINAL slots:
%     1:100   = H0
%     101:200 = H+50
%     201:300 = H-50
%
% Invalid permutation draws remain NaN and are tracked by permAccepted.
% They are NOT removed/reindexed before saving.
...
if nargin < 8 || isempty(numPerms), numPerms = 300; end
...
% Original permutation-index blocks for downstream Bayes comparison.
results.permBlocks.H0 = 1:100;
results.permBlocks.H50 = 101:200;
results.permBlocks.Hneg50 = 201:300;

if numPerms < 300
    warning(['numPerms=%d. This analysis can run, but the downstream ' ...
             'H0/H+50/H-50 Bayes comparison requires 300 permutations.'], ...
             numPerms);
end
...
for iDir = 1:nSess
    ...
    out.permBlocks.H0 = 1:100;
    out.permBlocks.H50 = 101:200;
    out.permBlocks.Hneg50 = 201:300;
    out.meta = meta;
    ...
    for bIdx = 1:nB
        ...
        % Preserve ALL original permutation slots. Model identity in the
        % downstream Bayes analysis depends on the ORIGINAL permutation
        % index, so invalid draws must not be removed/reindexed here.
        out.beh(bIdx).permPeakLags = permPeakLags;
        out.beh(bIdx).permAccepted = goodPerm;
