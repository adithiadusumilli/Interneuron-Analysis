function runIntPyrLagSkew5StdPermutationTest(saveFile, analysisType, nNullDraws)
% runs int v pyr only 5stdev significance analysis & skew permutation testing
% for no-chunk & chunked pairwise saved results

% updated logic:
%   - for each true int x pyr pair, compute null mean and null std from shifted controls
%   - classify pairs significant if real peak correlation > null mean + 5 * null std
...
%   - for null draws only, randomly assign each sampled pair a lag sign of +1 or -1
%     to account for the fact that reversing pair order flips lag sign
%   - compute significant lags and skew using the same 5-std rule
...
results.sigMethod = 'real_peakCorr_gt_nullMean_plus_5std';
...
for s = 1:nSess
    ...
    actual = computePairSetStats5Std(peakCorr, peakLag, nullCorr, actualRows, actualCols);

    fprintf('%s actual: nominal pairs = %d | valid tests = %d | sig 5std = %d | skew = %.6f\n', ...
        animalID, numel(actualRows), actual.nValidTests, actual.nSig5Std, actual.skew);
    ...
    for r = 1:nNullDraws
        ...
        % key null fix:
        % because upper-triangle ordering arbitrarily chooses one pair direction,
        % randomly assign each sampled pair a sign to emulate both orientations
        lagSignVec = ones(nActualNominalPairs,1);
        lagSignVec(rand(nActualNominalPairs,1) > 0.5) = -1;

        nullDraw = computePairSetStats5Std(peakCorr, peakLag, nullCorr, drawRows, drawCols, lagSignVec);

        nullSkews(r) = nullDraw.skew;
        nullNSig5Std(r) = nullDraw.nSig5Std;
    ...
    figure('Name', sprintf('%s actual int-pyr significant lags (5std)', animalID), 'Color', 'w');
    ...
    xlabel('Peak Lag (s)');
    ylabel('Count');
    title(sprintf('%s actual int-pyr 5std significant lags | sig=%d | skew=%.4f', ...
        animalID, actual.nSig5Std, actual.skew));
    grid on;

    pooledNullLags = vertcat(nullLagCell{:});
    pooledNullLags = pooledNullLags(~isnan(pooledNullLags) & isfinite(pooledNullLags));

    figure('Name', sprintf('%s null significant lags (5std)', animalID), 'Color', 'w');
    ...
    figure('Name', sprintf('%s skew null distribution (5std)', animalID), 'Color', 'w');
...
outFile = fullfile(folderPath, sprintf('%s_intPyrSkew5Std_%s.mat', baseName, char(analysisType)));
...
% ============================================================
function out = computePairSetStats5Std(realMat, lagMat, nullMat, rows, cols, lagSignVec)
% computes pairwise 5-std significance, significant lags, and skew
% for an arbitrary list of selected pairs

if nargin < 6 || isempty(lagSignVec)
...
for i = 1:nPairs
    ...
    nullStds(i) = thisStd;
    sigThresh(i) = thisMean + 5*thisStd;

    if isfinite(thisStd) && ~isnan(thisStd) && thisStd > 0
        sig5Std(i) = thisReal > sigThresh(i);
    end
end
