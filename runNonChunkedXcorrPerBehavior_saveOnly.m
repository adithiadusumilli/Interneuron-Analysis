...
% non-chunked (full time series) xc between cortex interneuron and pyramidal population activity, computed separately within each behavior label type

% save-only version:
%   - computes everything
%   - saves all plotting-ready outputs
%   - does not make figures!!! plotting code for this in github
...
% updated fix:
%   - shifted control correlations are now generated by shifting the FULL session-length interneuron population trace first, then restricting that shifted trace to the behavior timepoints
%   - this is for null logic to be more consistent w chunked analysis ( also avoids requiring each behavior to be >= ~60 s long just to allow a 30 s minimum shift )
...
%   runNonChunkedXcorrPerBehavior_saveOnly(baseDirs, "classifier", 0:10, 0.5, 100, 100);
    ...
    results.sessions = repmat(struct('baseDir', '', 'sessionTag', '','meta', struct(),'beh', []), 1, nSess);
    ...
    for iDir = 1:nSess
        ...
        for bIdx = 1:nB
            ...
            % FIXED VERSION: shift the FULL session-length int trace first then restrict the shifted trace to behavior timepoints
            % ============================================================
...
% classifier-only version!!!!!! no more manual/umap
% =====================================================================
...
%   meta: struct with mapping info and canonical names
    ...
    % lookup table for alternate behavior names across animals (found some inconsistent naming before, so easy handling now)
